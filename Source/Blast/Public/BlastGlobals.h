...
inline NvcQuat ToNvQuat(const FQuat& Quat)
{
	return { static_cast<float>(Quat.X), static_cast<float>(Quat.Y), static_cast<float>(Quat.Z), static_cast<float>(Quat.W) };
}

inline NvcQuat ToNvQuat(const FQuat4f& Quat)
{
	return { Quat.X, Quat.Y, Quat.Z, Quat.W };
}

inline NvcVec3 ToNvVector(const FVector& Vec)
{
	return { static_cast<float>(Vec.X), static_cast<float>(Vec.Y), static_cast<float>(Vec.Z) };
}

inline NvcVec3 ToNvVector(const FVector3f& Vec)
{
	return { Vec.X, Vec.Y, Vec.Z };
}

inline NvcVec2 ToNvVector(const FVector2f& Vec)
{
	return { Vec.X, Vec.Y };
}

inline void ToNvPlane4(const FPlane& Plane, float OutPlane[4])
{
...
inline void ToNvPlane4(const FPlane4f& Plane, float OutPlane[4])
{
	OutPlane[0] =  Plane.X;
	OutPlane[1] =  Plane.Y;
	OutPlane[2] =  Plane.Z;
