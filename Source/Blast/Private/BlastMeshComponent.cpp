	...
	BlastProxy(nullptr)
{
	...
	// bShouldCreateADefaultPhysicsState = false; // Not available in UE4.27
...
void UBlastMeshComponent::RebuildChunkVisibility()
{
	...
	// Send visible chunks to render thread for collision debug render 
	{
		...
		{
			ENQUEUE_RENDER_COMMAND(VisibleBonesForDebugDataCommand)([Proxy{ BlastProxy } , Chunks{ MoveTemp(VisibleChunks) }](FRHICommandList& RHICmdList) mutable
			{
				Proxy->UpdateVisibleChunks(MoveTemp(Chunks));
			});
		}
	}
#endif
...
#endif // !BLAST_DISABLE_STRESS_SOLVER
